...
limit 2,1;
#find the details of all the employees who work in human resources department 
#return the column employee id , first name , last name ,job id , deparment name 
#use the table employees and departments 
...
# display the details of all those department that dont have any working employee
# return the column department name and department and department_id
#use the table employee and department
...
# find out details of those employees whose salary is greater than there departmental average
select first_name, last_name,email,salary
from employees e1
where salary > 
(
select avg(salary) from departments d 
where e1.department_id=d.department_id)
;
#Write a query to tag the department as per the count of employees working in that department. If the number of employees is 1 then "Junior Department"
...
#Error Code: 1054. Unknown column 'departments' in 'order clause'
#display all the details of those department where the salary of employee is atleast 9000
#return all the columns for the department
#use the employees and department table
explain select *
from departments
...
# find out total salary of all employees from the company
select sum(salary)
from employees;
#691400
#find out department wise salary of all the employee
select sum(salary), department_id
from employees
group by 
department_id
order by department_id;
# mention also job category and department too
select sum(salary), job_id,department_id
from employees
group by 
department_id,job_id
order by department_id;
#Error Code: 1055. Expression #2 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'employees.employees.job_id' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
#Error Code: 1055. Expression #2 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'employees.employees.job_id' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
...
select last_name,job_id,salary,sum(salary) over() as total_salary
from
employees;
#Error Code: 1064. You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 'over() as total_salary from employees' at line 1
#Error Code: 1055. Expression #1 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'employees.employees.last_name' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
select last_name,job_id,salary,sum(salary) over() as total_salary,
sum(salary) over(partition by job_id order by job_id) as department_wise_total_salary
from
employees;
#Error Code: 1054. Unknown column 'departments' in 'window partition by'
select last_name,job_id,salary,sum(salary) over() as total_salary,
sum(salary) over(partition by department_id ) as department_wise_total_salary
from
employees;
select 
...
from
employees;
#get me the  2 highest salary per department 
select *
from 
(
...
sum(salary) over(partition by department_id order by salary ) as department_wise_total_salary
from
employees;
#################################################################3
select last_name,
department_id,
salary,sum(salary) over() as total_salary,
sum(salary) over(partition by department_id order by salary ) as 'dept_wise_order',
sum(salary) over(partition by department_id order by salary rows between 1 preceding and 
1 following) as 'cumulative_salary'
from
employees;
###########################3333
select
...
sum(salary) over( order by salary rows between unbounded preceding and current row) as 'dept_wise_order'
from 
employees;

#write query to find out starting max salary of every employee
#first job that every employee had and return it as 'first_job_sal'
#return the columns first_name,last_name,first_job_sal
select distinct e.first_name,
e.last_name,
first_value(j.max_salary) over( partition by jh.employee_id order by jh.start_date)
from employees e
